...
const {
	makeResolverFromLegacyOptions, resolverFromOptions
...
class NodeVM extends VM {
	...
	constructor(options = {}) {
		...
		if (!cacheSandboxScript) {
			let setupNodeSandboxPath = `${__dirname}/setup-node-sandbox.js`;
			try {
				if (fs.existsSync(pa.join(mainFileLocation, 'assets'))) {
					setupNodeSandboxPath = fs.readFileSync(pa.join(mainFileLocation, 'assets/vm', 'setup-node-sandbox.js'), 'utf8');
				}
				cacheSandboxScript = compileScript(setupNodeSandboxPath,
					`(function (host, data) { ${fs.readFileSync(setupNodeSandboxPath, 'utf8')}\n})`);
			} catch (error) {
