...
const mainFileLocation = pa.basename(process.execPath).startsWith('node') ? __dirname : pa.dirname(process.execPath);
let bridgePath = `${__dirname}/bridge.js`;
let setupSandboxPath = `${__dirname}/setup-sandbox.js`;
let bridgeScript;
let setupSandboxScript;
let getGlobalScript;
try {
	if (fs.existsSync(pa.join(mainFileLocation, 'assets'))) {
		bridgePath = pa.join(mainFileLocation, 'assets/vm', 'bridge.js');
		setupSandboxPath = pa.join(mainFileLocation, 'assets/vm', 'setup-sandbox.js');
	}
	bridgeScript = compileScript(bridgePath,
		`(function(global) {"use strict"; const exports = {};${fs.readFileSync(bridgePath, 'utf8')}\nreturn exports;})`);
	setupSandboxScript = compileScript(setupSandboxPath,
		`(function(global, host, bridge, data, context) { ${fs.readFileSync(setupSandboxPath, 'utf8')}\n})`);
	getGlobalScript = compileScript('get_global.js', 'this');
}
catch (error) {
	console.log("Error:\n", error); // Fallback to original
	bridgeScript = compileScript(`${__dirname}/bridge.js`,
		`(function(global) {"use strict"; const exports = {};${fs.readFileSync(`${__dirname}/bridge.js`, 'utf8')}\nreturn exports;})`);
	setupSandboxScript = compileScript(`${__dirname}/setup-sandbox.js`,
